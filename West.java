...
class West extends JPanel implements ActionListener{
	...
	JLabel nLabel = new JLabel("WEST INDIA");
	JLabel pLabel = new JLabel("Packages of West India");
	...
	ImageIcon icon = new ImageIcon("West1.png");
	...
	public void actionPerformed(ActionEvent fd){
		...
		try{
			...
			PreparedStatement ps = con.prepareStatement("select fare from West where name=?");
			ps.setString(1,s);
			ResultSet rs = ps.executeQuery();
			if(rs.next())
			{
		...
		try{
			...
			PreparedStatement ps = con.prepareStatement("select stay,places from west where name=?");
			ps.setString(1,s);
			ResultSet rs = ps.executeQuery();
			if(rs.next())
			{
