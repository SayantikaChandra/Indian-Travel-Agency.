...
class Bookn extends JPanel implements ActionListener{
	...
	Bookn (JFrame fr,String dest) {
		...
		bkButton.setBounds(120,600,150,50);
		baButton.setBounds(320,600,150,50);
		logButton.setBounds(520,600,150,50);
		...
		try{
			...
			PreparedStatement ps = con.prepareStatement("select hotel from north where name=?");
			ps.setString(1,dest);
	...
	public void actionPerformed(ActionEvent fd){
		...
		if(src==baButton){
			...
			JFrame frame=new JFrame("NORTH INDID");
			frame.setSize(1300,750);
			North mp = new North(frame);
		...
		else if(src==dem)
		{
			...
			try{
				...
				PreparedStatement ps = con.prepareStatement("select fare from north where name=?");
				ps.setString(1,dest);
				ResultSet rs = ps.executeQuery();
				if(rs.next())
				{
